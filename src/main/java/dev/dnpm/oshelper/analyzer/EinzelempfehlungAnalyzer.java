...
/**
 * Diese Klasse implementiert ein Plugin, welches Funktionen für DNPM UF Einzelempfehlung
 * bereitstellt.
 *
 * @since 0.2.0
 */
...
public class EinzelempfehlungAnalyzer extends BackendService {
  ...
  private static final Logger logger = LoggerFactory.getLogger(EinzelempfehlungAnalyzer.class);
  ...
  public List<Variant> getVariants(Map<String, Object> input) {
    ...
    if (permissionEvaluator.hasPermission(
        SecurityContextHolder.getContext().getAuthentication(), procedure, PermissionType.READ)) {
  ...
  /**
   * Übergibt alle Studien, deren (Kurz-)Beschreibung oder NCT-Nummer den übergebenen Eingabewert
   * <code>q</code> enthält
   *
