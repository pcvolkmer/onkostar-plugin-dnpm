...
/**
 * Diese Klasse implementiert ein Plugin, welches Aktionen nach Bearbeitung eines FollowUps
 * durchführt.
 *
 * @since 0.0.2
 */
...
public class FollowUpAnalyzer extends Analyzer {
  ...
  private void backlinkToEinzelempfehlung(Procedure procedure) {
    ...
    if (null == referencedProcedure) {
      logger.error(
          "Referenzierte Einzelempfehlung wurde nicht gefunden: {}", referencedProcedureId);
      return;
    }

    referencedProcedure.setValue(
        "refdnpmfollowup", new Item("ref_dnpm_followup", procedure.getId()));
