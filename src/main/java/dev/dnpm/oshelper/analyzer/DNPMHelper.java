...
public class DNPMHelper extends BackendService {
  ...
  public List<Map<String, String>> getVerweise(final Map<String, Object> input) {
    ...
    try {
      ...
      String sql =
          "SELECT prozedur.id AS procedure_id, prozedur.data_form_id, data_catalogue.name AS data_catalogue, data_catalogue_entry.name AS data_catalogue_entry, data_form.description AS formname, prozedur.beginndatum AS datum "
              + "FROM prozedur "
      ...
      try {
        ...
        for (VerweisVon verweisVon : result) {
          ...
          query = session.createSQLQuery(sql).addScalar("value", StandardBasicTypes.INTEGER);
  ...
  public String getProzedurenFromDiagnose(final Map<String, Object> input) {
    ...
    for (Procedure Prozedur : prozeduren) {
      ...
      if (Prozedur.getDiseaseIds().contains(diagnoseId.get())
          && Prozedur.getFormName().contains(dataForm.get())) {
        // alle Werte auslesen
        ...
        formulare.add(
            Map.of(
                ...
                "Felder",
                new HashMap<>(Prozedur.getAllValues())));
      }
    }
    try {
  ...
  public Object getEmpfehlung(final Map<String, Object> input) {
    ...
    try {
      ...
      Session session = sessionFactory.getCurrentSession();
      var sql =
          "SELECT prozedur.id, genname, geneid, geneidlink, empfehlung, beginndatum FROM prozedur "
  ...
  public Object updateEmpfehlungPrio(final Map<String, Object> input) {
    ...
    try {
      String sql =
          "UPDATE prozedur SET beginndatum = '"
              + strDate.get()
              + "' WHERE id = '"
              + rid.get()
              + "' ";
  ...
  public List<EcogStatusWithDate> getEcogStatus(final Map<String, Object> input) {
    ...
    if (delegatingDataBasedPermissionEvaluator.hasPermission(
        SecurityContextHolder.getContext().getAuthentication(), patient, PermissionType.READ)) {
