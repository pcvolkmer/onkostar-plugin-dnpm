...
public class DefaultSystemtherapieService implements SystemtherapieService {
  ...
  public DefaultSystemtherapieService(
  ...
  /**
   * Übergibt aktuell immer den Mapper für das Formular "OS.Systemische Therapie", da beide bekannte
   * Varianten damit gemappt werden können.
   *
  ...
  public List<EcogStatusWithDate> ecogStatus(Patient patient) {
    return patient.getDiseases().stream()
        .flatMap(
            disease ->
                onkostarApi.getProceduresForDiseaseByForm(disease.getId(), getFormName()).stream())
        .filter(procedure -> null != procedure.getStartDate())
        .sorted(Comparator.comparing(Procedure::getStartDate))
        .map(
            procedure -> {
              try {
                return new EcogStatusWithDate(
                    procedure.getStartDate(), procedure.getValue(ECOG_FIELD).getString());
  ...
  private String getFormName() {
    return settingsService.getSetting("systemtherapieform").orElse("OS.Systemische Therapie");
  }
}
