...
public class OsTumorkonferenzVarianteUkwToProtocolMapper implements ProcedureToProtocolMapper {
  ...
  /**
   * Wandelt eine Prozedur mit Formularnamen "OS.Tumorkonferenz.VarianteUKW" in ein {@link Optional}
   * mit einer Zeichenkette oder im Fehlerfall in ein leeres Optional um.
   *
  ...
  public Optional<String> apply(Procedure procedure) {
    ...
    if (null != fragestellung
        && !fragestellung.getString().isBlank()
        && null != empfehlung
        && !empfehlung.getString().isBlank()) {
      return Optional.of(
          String.format(
              "Fragestellung:%n%s%n%nEmpfehlung:%n%s",
              fragestellung.getString().trim(), empfehlung.getString().trim()));
