...
public class DefaultTherapieplanService extends AbstractTherapieplanService {
  ...
  /**
   * Verlinke MTB und Übernahme Datum aus Hauptformular in weiteren Bereichen "Humangenetische
   * Beratung" und "Reevaluation" und Unterformularen, wenn erforderlich.
   *
  ...
  public List<Procedure> findReferencedMtbs(Procedure procedure) {
    ...
    var mtbProcedure =
        this.onkostarApi.getProcedure(procedure.getValue(FORMFIELD_REFERSTEMTB).getInt());
  ...
  private void updateMtbInSections(Procedure procedure) {
    if (!isYes(procedure, FORMFIELD_HUMANGENBERATUNG)
    ...
    if (isYes(procedure, FORMFIELD_HUMANGENBERATUNG)
      ...
      procedure.setValue(
          FORMFIELD_REFTKHUMANGENBER, new Item(DATAFIELD_REF_TK_HUMANGENBER, mtbReference));
    ...
    if (isYes(procedure, FORMFIELD_HUMANGENBERATUNG)
      ...
      procedure.setValue(
          FORMFIELD_DATUMTKHUMANGENBER, new Item(DATAFIELD_DATUM_TK_HUMANGENBER, mtbDate));
      noUpdateRequired = false;
    }

    if (isYes(procedure, FORMFIELD_REEVALUATION)
        && (!hasValue(procedure, FORMFIELD_REFTKREEVALUATION)
            || mtbReference != procedure.getValue(FORMFIELD_REFTKREEVALUATION).getInt())) {
      procedure.setValue(
          FORMFIELD_REFTKREEVALUATION, new Item("ref_tk_reevaluation", mtbReference));
      noUpdateRequired = false;
    }
  ...
  private void updateMtbInSubforms(Procedure procedure) {
    if (!hasValue(procedure, FORMFIELD_REFERSTEMTB) || !hasValue(procedure, FORMFIELD_DATUM)) {
      return;
    }
            ...
            subform -> {
              if (isUsableEinzelempfehlung(subform, mtbReference, mtbDate)) {
                ...
                try {
                  ...
                  logger.error(
                      "Formular 'DNPM UF Einzelempfehlung' konnte nicht aktualisiert werden", e);
                }
              }

              if (isUsableRebiopsie(subform, mtbReference, mtbDate)) {
                subform.setValue(
                    FORMFIELD_REFTUMORKONFERENZ, new Item("ref_tumorkonferenz", mtbReference));
  ...
  private static boolean isUsableRebiopsie(Procedure subform, int mtbReference, Date mtbDate) {
    return subform.getFormName().equals("DNPM UF Rebiopsie")
        && mtbReference != subform.getValue(FORMFIELD_REFTUMORKONFERENZ).getInt()
        && !mtbDate.equals(subform.getValue(FORMFIELD_UFRBDATUM).getDate());
  }

  private static boolean isUsableEinzelempfehlung(
      Procedure subform, int mtbReference, Date mtbDate) {
    return subform.getFormName().equals("DNPM UF Einzelempfehlung")
        && mtbReference != subform.getValue(FORMFIELD_MTB).getInt()
        && !mtbDate.equals(subform.getValue(FORMFIELD_UFEEDATUM).getDate());
  }
}
