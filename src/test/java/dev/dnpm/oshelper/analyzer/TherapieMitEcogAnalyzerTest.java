...
class TherapieMitEcogAnalyzerTest {
  ...
  void setUp(
    ...
    this.therapieMitEcogAnalyzer =
        new TherapieMitEcogAnalyzer(onkostarApi, strahlentherapieService, systemtherapieService);
  }
  ...
  void shouldInsertNewEcogStatus() throws Exception {
    ...
    doAnswer(invocationOnMock -> List.of(new EcogStatusWithDate(ecogDate, "0")))
        .when(systemtherapieService)
        .ecogStatus(any(Patient.class));
    ...
    doAnswer(invocationOnMock -> List.of(dummyDisease(42, diagnosisDate)))
        .when(this.onkostarApi)
        .getDiseasesByPatientId(anyInt());

    doAnswer(invocationOnMock -> List.of(procedure))
        .when(onkostarApi)
    ...
    verify(onkostarApi, times(1))
        .getProceduresForDiseaseByForm(idCaptor.capture(), formNameCaptor.capture());
