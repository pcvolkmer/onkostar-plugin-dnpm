...
class DefaultTherapieplanServiceTest {
  ...
  @BeforeEach
  void setUp(@Mock IOnkostarApi onkostarApi, @Mock FormService formService) {
  ...
  void shouldNotUpdateSectionsIfSectionsNotEnabled() throws Exception {
    ...
    testProcedure.setValue(
        "humangenberatung", new Item("humangen_beratung", JaNeinUnbekannt.NEIN.getCode()));
    testProcedure.setValue(
        "reevaluation", new Item("reevaluation", JaNeinUnbekannt.NEIN.getCode()));
  ...
  void shouldUpdateSectionsIfNoReevaluation() throws Exception {
    ...
    testProcedure.setValue(
        "humangenberatung", new Item("humangen_beratung", JaNeinUnbekannt.JA.getCode()));
    testProcedure.setValue(
        "humangenberbegruendung", new Item("humangen_ber_begruendung", "Das ist die Begründung"));
    testProcedure.setValue(
        "reevaluation", new Item("reevaluation", JaNeinUnbekannt.NEIN.getCode()));
  ...
  void shouldFindFollowUps() {
    ...
    doAnswer(
            invocationOnMock -> {
              ...
              procedure.setValue(
                  "LinkTherapieempfehlung", new Item("LinkTherapieempfehlung", procedureId));
              return List.of(procedure);
            })
        .when(onkostarApi)
        .getProceduresForDiseaseByForm(anyInt(), anyString());
